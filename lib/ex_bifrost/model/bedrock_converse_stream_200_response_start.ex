# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.BedrockConverseStream200ResponseStart do
  @moduledoc """
  BedrockConverseStream200ResponseStart model.
  """

  @derive JSON.Encoder
  defstruct [
    :toolUse
  ]

  @type t :: %__MODULE__{
          :toolUse => ExBifrost.Model.BedrockConverseStream200ResponseStartToolUse.t() | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:toolUse, :struct, ExBifrost.Model.BedrockConverseStream200ResponseStartToolUse)
  end
end
