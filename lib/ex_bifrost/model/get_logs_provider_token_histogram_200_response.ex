# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.GetLogsProviderTokenHistogram200Response do
  @moduledoc """
  Time-bucketed token histogram with provider breakdown
  """

  @derive JSON.Encoder
  defstruct [
    :buckets,
    :bucket_size_seconds,
    :providers
  ]

  @type t :: %__MODULE__{
          :buckets => [ExBifrost.Model.GetLogsProviderTokenHistogram200ResponseBucketsInner.t()] | nil,
          :bucket_size_seconds => integer() | nil,
          :providers => [String.t()] | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:buckets, :list, ExBifrost.Model.GetLogsProviderTokenHistogram200ResponseBucketsInner)
  end
end
