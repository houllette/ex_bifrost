# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.ChatMessageAnnotationsInner do
  @moduledoc """
  ChatMessageAnnotationsInner model.
  """

  @derive JSON.Encoder
  defstruct [
    :type,
    :url_citation
  ]

  @type t :: %__MODULE__{
          :type => String.t() | nil,
          :url_citation => ExBifrost.Model.ChatMessageAnnotationsInnerUrlCitation.t() | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:url_citation, :struct, ExBifrost.Model.ChatMessageAnnotationsInnerUrlCitation)
  end
end
