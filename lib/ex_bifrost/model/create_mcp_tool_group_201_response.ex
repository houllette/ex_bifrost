# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.CreateMcpToolGroup201Response do
  @moduledoc """
  CreateMcpToolGroup201Response model.
  """

  @derive JSON.Encoder
  defstruct [
    :mcp_tool_group
  ]

  @type t :: %__MODULE__{
          :mcp_tool_group => ExBifrost.Model.ListMcpToolGroups200ResponseMcpToolGroupsInner.t() | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(
      :mcp_tool_group,
      :struct,
      ExBifrost.Model.ListMcpToolGroups200ResponseMcpToolGroupsInner
    )
  end
end
