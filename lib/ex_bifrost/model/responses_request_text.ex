# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.ResponsesRequestText do
  @moduledoc """
  ResponsesRequestText model.
  """

  @derive JSON.Encoder
  defstruct [
    :format,
    :verbosity
  ]

  @type t :: %__MODULE__{
          :format => ExBifrost.Model.ResponsesRequestTextFormat.t() | nil,
          :verbosity => String.t() | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(:format, :struct, ExBifrost.Model.ResponsesRequestTextFormat)
  end
end
