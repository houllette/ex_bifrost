# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.GeminiGenerationResponseCandidatesInnerUrlContextMetadata do
  @moduledoc """
  GeminiGenerationResponseCandidatesInnerUrlContextMetadata model.
  """

  @derive JSON.Encoder
  defstruct [
    :urlMetadata
  ]

  @type t :: %__MODULE__{
          :urlMetadata =>
            [ExBifrost.Model.GeminiGenerationResponseCandidatesInnerUrlContextMetadataUrlMetadataInner.t()] | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(
      :urlMetadata,
      :list,
      ExBifrost.Model.GeminiGenerationResponseCandidatesInnerUrlContextMetadataUrlMetadataInner
    )
  end
end
