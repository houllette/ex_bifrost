# NOTE: This file is auto generated by OpenAPI Generator 7.23.0 (https://openapi-generator.tech)
# using the Elixir SDK Generator template (https://github.com/houllette/elixir-sdk-generator).
# Do not edit this file manually — edit the templates and regenerate instead.

defmodule ExBifrost.Model.GeminiGenerationRequestGenerationConfigSpeechConfigMultiSpeakerVoiceConfig do
  @moduledoc """
  GeminiGenerationRequestGenerationConfigSpeechConfigMultiSpeakerVoiceConfig model.
  """

  @derive JSON.Encoder
  defstruct [
    :speakerVoiceConfigs
  ]

  @type t :: %__MODULE__{
          :speakerVoiceConfigs =>
            [
              ExBifrost.Model.GeminiGenerationRequestGenerationConfigSpeechConfigMultiSpeakerVoiceConfigSpeakerVoiceConfigsInner.t()
            ]
            | nil
        }

  alias ExBifrost.Deserializer

  def decode(value) do
    value
    |> Deserializer.deserialize(
      :speakerVoiceConfigs,
      :list,
      ExBifrost.Model.GeminiGenerationRequestGenerationConfigSpeechConfigMultiSpeakerVoiceConfigSpeakerVoiceConfigsInner
    )
  end
end
